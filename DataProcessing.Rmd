...
imp <- na.interpolation(sapply(as.data.frame(gwlevel[,11]), function(y) y*0.3048),option = 'linear')
ggplot_na_imputations(sapply(as.data.frame(gwlevel[,11]), function(y) y*0.3048),imp,
                      xlab="Months",ylab = "Water level (m)",
                      subtitle = colnames(gwlevel)[11])
```
```{r,echo = FALSE, warning=FALSE}
